...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    let responses: { question_label: string; answer_text: string; audio_url: string | null }[] = []
    ...
    const answersHtml =
      responses.length > 0
        ? responses
            .map((r) => {
              const audioHtml = r.audio_url
                ? `<br><a href="${r.audio_url}" target="_blank" style="display:inline-flex;align-items:center;gap:4px;color:#a6907f;font-size:13px;text-decoration:none;margin-top:6px;font-weight:600;">\u25B6 Ouvir \u00E1udio gravado</a>`
                : ''
              return `
            <tr>
              <td style="padding:8px 12px;border-bottom:1px solid #eee;font-weight:600;width:45%;vertical-align:top;">${r.question_label}</td>
              <td style="padding:8px 12px;border-bottom:1px solid #eee;white-space:pre-wrap;">${r.answer_text || '\u2014'}${audioHtml}</td>
            </tr>`
            })
            .join('')
